...
for day in unique_days:
    ...
    morning_price_curve = resampled_day['Close'].values[:MORNING_PERIOD_MINUTES]
    rolling_vol = pd.Series(morning_price_curve).rolling(window=10).std().dropna()
    vol_of_vol = np.std(rolling_vol)
    zero_crossings = np.sum(np.diff(np.sign(morning_curve)) != 0)
    morning_df = resampled_day.iloc[:MORNING_PERIOD_MINUTES]
    if morning_df['Volume'].sum() > 0:
        morning_vwap = (morning_df['Close'] * morning_df['Volume']).sum() / morning_df['Volume'].sum()
        # Normalize the deviation by the VWAP itself
        vwap_deviation = (morning_df['Close'].iloc[-1] - morning_vwap) / morning_vwap
    else:
        vwap_deviation = 0 # No volume, no deviation
...
def train_and_evaluate_model(X, y, feature_names, ticker):
    """
...
advanced_scalar_names = [
    'Volatility', 'Return', 'Gap', 'Slope', 'MaxDrawdown', 'SPY_Return', 'SPY_Vol', 'VolOfVol', 'ZeroCrossings', 'VWAP_Dev'
]
...
for name, features in features_to_test.items():
    ...
    mae, fig = train_and_evaluate_model(features["X"], y, features["names"], TICKER_SYMBOL)
    
    experiment_results.append({'Feature Set': name, 'MAE': mae})
    
    # Show the plot for each experiment
    # fig.show() # You can uncomment this to see the plot for every single test
...
# Optional: Show the plot for the best performing model
best_set_name = final_results_df.iloc[0]['Feature Set']
print(f"\nDisplaying plot for the best performing feature set: '{best_set_name}'")
mae, best_fig = train_and_evaluate_model(
    features_to_test[best_set_name]["X"], 
    y, 
    features_to_test[best_set_name]["names"], 
    TICKER_SYMBOL
)
best_fig.show()